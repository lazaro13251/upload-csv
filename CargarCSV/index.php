  ...
  if (move_uploaded_file($csv_file, $csv_file_destination)) {
    ...
    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
      ...
      if (mysqli_num_rows($result) == 1) {
        // El ID existe, realizar una actualización
        $query = "UPDATE $table_name SET nombre = '$nombre', descripcion = '$descripcion', precio = $precio, cantidad = $cantidad WHERE id = $id";
        echo $query . "<br>";
        mysqli_query($conn, $query);
      } else {
        // El ID no existe, insertar como nuevo
        $query = "INSERT INTO $table_name (id, nombre, descripcion, precio, cantidad) VALUES (NULL, '$nombre', '$descripcion', $precio, $cantidad)";
        echo $query . "<br>";
        mysqli_query($conn, $query);
      }
    }
